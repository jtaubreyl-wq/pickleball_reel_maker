# FILE: src/visualisation/frame_overlay_gpu_test.py
...
class FrameOverlayGPU:
    """
    Applies a transparent PNG frame overlay to a video and guarantees
    a Messenger‑compatible MP4 output (H.264 + yuv420p).

    Pipeline:
    - Load PNG with alpha
    - Auto-crop transparent padding
    - Pad to perfect 16:9
    - Scale + center-crop to match video
    - Alpha-blend onto each frame
    - Write temporary MP4 (OpenCV)
    - Re-encode final MP4 using FFmpeg (H.264 + yuv420p)
    """
    ...
    # ------------------------------------------------------------
    # FFmpeg RE-ENCODE (Messenger-compatible)
    # ------------------------------------------------------------

    def _ffmpeg_reencode(self, temp_path, final_path):
        """
        Re-encode using FFmpeg to guarantee:
        - H.264 video
